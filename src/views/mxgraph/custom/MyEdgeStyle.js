 ...
 * @param result
 */
export const BezierEdgeStyle = (state, source, target, points, result) => {
    if (source != null && target != null) {
        ...
        const bps = createBezierPoints(basePoints, Math.abs(source.x + source.width - target.x) <= 100 &&
            Math.abs(source.y + source.height - target.y) <= 100
            ? 20
            : 40 *
                Math.floor(Math.max(Math.abs(source.x + source.width - target.x), Math.abs(source.y + source.height - target.y)) / 100));
        bps.forEach((p) => {
            // 把在source 和 target中的点去掉
            if (!mi.mxUtils.contains(source, p.x, p.y) &&
...
//# sourceMappingURL=MyEdgeStyle.js.map