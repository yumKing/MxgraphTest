...
export function createGraph(container) {
    ...
    keyHandler.keyDown = function (evt) {
        console.log('keydown, code: ', evt.keyCode, evt.key, // 一般键
...
export function createNode(graph, info) {
    ...
    try {
        ...
        cell = graph.insertVertex(parent, info.id, info.content, info.xpos, info.ypos, 180, 30);
    }
    finally {
...
export function createRelation(graph, info) {
    const parent = graph.getDefaultParent();
    graph.getModel().beginUpdate();
    let cell = {};
    try {
